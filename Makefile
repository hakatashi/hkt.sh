...
home/home.zip: home/main.go home/home.html.tpl
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o home/home ./home
...
entry/entry.zip: entry/main.go
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o entry/entry ./entry
	@if [ -f /cygdrive/c/Users/denjj/go/bin/build-lambda-zip.exe ]; then \
...
put-entry/put-entry.zip: put-entry/main.go
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o put-entry/put-entry ./put-entry
	@if [ -f /cygdrive/c/Users/denjj/go/bin/build-lambda-zip.exe ]; then \
